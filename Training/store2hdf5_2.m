function [curr_dat_sz, curr_lab_sz] = store2hdf5_2(filename, data, labels, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/data_2', [dat_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/label_2', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
  ...
  if ~isempty(data)
    h5write(filename, '/data_2', single(data), startloc.dat, size(data));
    h5write(filename, '/label_2', single(labels), startloc.lab, size(labels));  
  end
