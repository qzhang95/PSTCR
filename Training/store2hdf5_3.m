function [curr_dat_sz, curr_lab_sz] = store2hdf5_3(filename, data, labels, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/data_3', [dat_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/label_3', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
  ...
  if ~isempty(data)
    h5write(filename, '/data_3', single(data), startloc.dat, size(data));
    h5write(filename, '/label_3', single(labels), startloc.lab, size(labels));  
  end
